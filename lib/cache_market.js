...
export async function saveHistory(coin, range, data) {
  console.log("[MONITOR] CACHE WRITE");
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        runtimeMonitor.cacheWrites++;
        console.log("[MONITOR] cacheWrites:", runtimeMonitor.cacheWrites);
        resolve();
      };
      request.onerror = () => reject(request.error);
    });
...
export async function loadHistory(coin, range) {
  console.log("[MONITOR] CACHE READ");
  try {
    const db = await getDB();
    const key = getKey("history", coin, range);
    ...
    return await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        runtimeMonitor.cacheReads++;
        const result = request.result || null;
        console.log("[MONITOR] cacheReads:", runtimeMonitor.cacheReads);
        resolve(result);
      };
      request.onerror = () => reject(request.error);
    });
