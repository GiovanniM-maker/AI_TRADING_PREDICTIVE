...
if (typeof window !== "undefined") {
  console.log("[CACHE] Monitor ref:", monitor);
}
...
export async function saveHistory(coin, range, data) {
  console.log("[CACHE] saveHistory called", { coin, range, dataLength: data?.length });
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        monitor.cacheWrites++;
        console.log("[CACHE] saveHistory success - cacheWrites:", monitor.cacheWrites, "monitor:", monitor);
        resolve();
      };
      request.onerror = () => reject(request.error);
    });
...
export async function loadHistory(coin, range) {
  console.log("[CACHE] loadHistory called", { coin, range });
  try {
    const db = await getDB();
    const key = getKey("history", coin, range);
    ...
    return await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log("[CACHE] loadHistory success - cacheReads:", monitor.cacheReads, "result length:", result?.length, "monitor:", monitor);
        resolve(result);
      };
      request.onerror = () => reject(request.error);
    });
...
export async function saveIndicators(coin, range, data) {
  console.log("[CACHE] saveIndicators called", { coin, range, dataLength: data?.length });
  try {
    const db = await getDB();
    ...
    await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        monitor.cacheWrites++;
        console.log("[CACHE] saveIndicators success - cacheWrites:", monitor.cacheWrites, "monitor:", monitor);
        resolve();
      };
      request.onerror = () => reject(request.error);
    });
...
export async function loadIndicators(coin, range) {
  console.log("[CACHE] loadIndicators called", { coin, range });
  try {
    const db = await getDB();
    const key = getKey("indicators", coin, range);
    const transaction = db.transaction([STORE_NAME], "readonly");
    ...
    return await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        monitor.cacheReads++;
        const result = request.result || null;
        console.log("[CACHE] loadIndicators success - cacheReads:", monitor.cacheReads, "result length:", result?.length, "monitor:", monitor);
        resolve(result);
      };
      request.onerror = () => reject(request.error);
    });
